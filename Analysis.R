...
dat3<-dat
dat3 <-cbind(dat3,c(rowMeans(dat)))
dat3 <-data.frame(dat3)
names(dat3)[names(dat3) == 'V272'] <- 'log2fold'
dat3$log2fold

up <- rownames(dat3)[which(dat3$log2fold>1)]
down <- rownames(dat3)[which(dat3$log2fold<(1*-1))]


with(dat3, plot(log2fold, -log10(pval.t2), pch=20, main="Volcano plot", ylim = c(0,15),col = "grey"))

with(subset(dat3 , padj<.1), points(log2FoldChange, -log10(pvalue), pch=20,col="grey"))
with(res[which(rownames(dat3) %in% up),], points(log2FoldChange, -log10(pval.t2), pch=20, col="#0F7F7F"))
"with(res[which(rownames(dat3) %in% down),], points(log2FoldChange, -log10(pval.t2), pch=20, col="lightcoral"))
